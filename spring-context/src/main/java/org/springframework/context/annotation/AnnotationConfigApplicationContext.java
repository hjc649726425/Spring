...
public class AnnotationConfigApplicationContext extends GenericApplicationContext implements AnnotationConfigRegistry {
	...
	public AnnotationConfigApplicationContext() {
		//父类的默认构造方法初始化beanFactory

		//在IOC容器中初始化一个 注解bean读取器AnnotatedBeanDefinitionReader
		//注册内置注解解析器，用于注解BeanDefinition的读取解析
		//完成了spring内部BeanDefinition的注册（主要是后置处理器）

		//在IOC容器中初始化一个 按类路径扫描注解bean的 扫描器
		//创建BeanDefinition扫描器
		//可以用来扫描包或者类，继而转换为bd
		//主要用来加载解析 @Configuration 修改的配置类
		this.reader = new AnnotatedBeanDefinitionReader(this);

		//spring默认的扫描包不是这个scanner对象
		//而是自己new的一个ClassPathBeanDefinitionScanner
		//spring在执行工程后置处理器ConfigurationClassPostProcessor时，
		// 去扫描包时会new一个ClassPathBeanDefinitionScanner
	...
	public AnnotationConfigApplicationContext(Class<?>... componentClasses) {
		//初始化 AnnotatedBeanDefinitionReader和ClassPathBeanDefinitionScanner 对象
		this();
