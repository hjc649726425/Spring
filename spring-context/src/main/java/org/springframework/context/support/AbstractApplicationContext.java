...
public abstract class AbstractApplicationContext extends DefaultResourceLoader
	...
	public void refresh() throws BeansException, IllegalStateException {
		synchronized (this.startupShutdownMonitor) {
			// Prepare this context for refinvokeBeanFactoryPostProcessorsreshing.
			//刷新前的预处理
			...
			//BeanFactory的预准备工作（BeanFactory进行一些设置，比如context的类加载器，
			// BeanPostProcessor和XXXAware自动装配等）
			...
			try {
				...
				//执行bean工厂的后置处理器
				//spring是先调用内置的BeanFactoryPostProcessor
				// 再调用程序员自定义的BeanFactoryPostProcessor
				//先执行postProcessBeanDefinitionRegistry，再执行postProcessBeanFactory
	...
	protected void prepareBeanFactory(ConfigurableListableBeanFactory beanFactory) {
		...
		// ResourceLoader、ApplicationEventPublisher、ApplicationContext
	...
	/**
	 注册监听器的流程如下：
	 1.先将手动添加的监听器放到广播器中，我们可以调用AbstractApplicationContext#addApplicationListener添加监听器；
	 2.从beanFactory中获取取监听器的名称，添加到广播器中，
	 	注意：此时beanFactory中的bean并没有初始化，因此只能添加 beanName;
	 3.如果存在早期事件，就发布早期事件。
	 */
