...
public abstract class AbstractAutoProxyCreator extends ProxyProcessorSupport
	...
	//保存不需要代理的类
	//比如定义的切面本身是不需要被代理的，还有加了@Configuration注解的Config配置类
	...
	//就是放入集合里，然后判断要不要包装，其实就是在循环依赖注入属性的时候如果有AOP代理的话，
	// 也会进行代理，然后返回。
	@Override
	...
	protected Object wrapIfNecessary(Object bean, String beanName, Object cacheKey) {
		...
		// 判断这个bean是不是有没有实现Advice、PuintCut、Advisor、AopInfrastructureBean接口
		...
		// 获取切面和切面方法,获取配置好的Advisor信息
		//1.获取到项目中所有的切面对象，将其中的切面方法封装为一个List<Advisor>，在postProcessBeforeInstantiation中已经处理了；
		//2.遍历Advisor，对每一个Advisor，利用反射获取当前类的除Object外的所有父类及接口，结果为Set<Class>；
		//3.遍历Set<Class>，对其中每一个Class，利用反射获取该Class方法、除Object外的所有父类的方法、接口的默认方法，结果为Method[];
		//4.遍历Method[]，如果有一个method满足Advisor的切面条件，则表示当前Advisor可以应用到当前bean，该bean就需要被代理，这一步最终得到的结果也是一个List<Advisor>，表示该有多个Advisor 需要应用到该对象。
