...
public abstract class TransactionAspectSupport implements BeanFactoryAware, InitializingBean {
	...
	protected Object invokeWithinTransaction(Method method, @Nullable Class<?> targetClass,
		...
		// 一般这里都是DataSourceTransactiuonManager
	 ...
	 */
	//1.如果在 @Transaction 注解上指定了事务管理器，就从spring容器中获取该事务管理器
	//2.如果指定事务管理器名称，就从spring容器中获取该事务管理器
	//3.如果指定事务管理器对象，直接返回
	//4.以上都不满足，直接从spring容器中获取类型为 TransactionManager 的bean
	@Nullable
	...
	/**
	 TransactionAspectSupport中有一个ThreadLocal，用来存放当前的TransactionInfo对象，
	 进行线程绑定时，会先拿到旧的事务信息，保存在TransactionInfo的成员变量oldTransactionInfo中，
	 然后将新的TransactionInfo放入ThreadLocal中；
	 当事务执行完成后，会从TransactionInfo的成员变量oldTransactionInfo中拿到旧的事务信息，
	 再将旧的事务信息放入ThreadLocal中，这就完成了事务"旧-新-旧"的切换.
	 */
	...
	protected final class TransactionInfo {
		...
		@Nullable
		//事务属性值，用来保存@Transactional注解的属性值
		private final TransactionAttribute transactionAttribute;

		// 方法的全限定名（格式为："包名.类型.方法名"）
		private final String joinpointIdentification;

		// 当前的事务状态对象
		//从名称上看，是记录事务的状态，实际这个对象不仅记录了事务的状态，包括的重大功能如下：
		//complete: 事务的完成状态
