...
public abstract class AbstractHandlerMapping extends WebApplicationObjectSupport
	...
	/**
	 ...
	 2.获取 executionChain，这个executionChain除了包含了上一步的handler外，
	 	还包含uri 对应的 Interceptors，获取方法为获取所有的Interceptors配置
	 	（在WebMvcConfigurationSupport中配置的），再逐一判断uri是否符合Interceptor的uri配置；
	 3.获取cors跨域配置，然后添加到executionChain中的Interceptors列表的第一位。
	 	嗯，没错，cors跨域配置也是在WebMvcConfigurationSupport中配置的。
	 */
	@Override
	@Nullable
