...
public class DispatcherServlet extends FrameworkServlet {
	...
	private void initThemeResolver(ApplicationContext context) {
		try {
			// 从容器获取名称为 themeResolver 类型为 ThemeResolver 的策略组件
	...
	private void initHandlerAdapters(ApplicationContext context) {
		...
		if (this.detectAllHandlerAdapters) {
			...
			try {
				// 从容器中获取 HandlerAdapter ，如果获取不到 下面则会添加默认的..
		...
		//通过注册，确保至少有一个 HandlerAdapter
		//如果找不到其他映射，则为默认的 HandlerAdapter。
	 ...
	 */
	/***
	 1.获取对应的HandlerExecutionChain, 获取的HandlerExecutionChain中包含
	 	真正地处理器（Controller中的方法和一组HandlerInterceptor拦截器；
	...
	protected void doDispatch(HttpServletRequest request, HttpServletResponse response) throws Exception {
		...
		try {
			...
			try {
				...
				// 不同的Controller用不同的适配器
				//@Controller  实现Controller接口   实现HttpRequestHandler接口
				...
				// 4. 通过上面获取到的handlerAdapter来调用handle,拿到ModelAndView
				...
				//在执行完handler方法后之后，在视图解析之前。
				// 因此，我们可以在拦截器的这个方法里处理一些额外的渲染参数。
	...
	/**
	 这个方法做了三件事:
	 1. 异常处理：如果前面的执行中出现了异常，在这个方法里会查找项目中配置的处理器进行处理；
	 2.视图渲染：渲染返回的视图
	 3.执行拦截器方法：HandlerInterceptor#afterCompletion
	 */
		...
		if (this.handlerMappings != null) {
			for (HandlerMapping mapping : this.handlerMappings) {
				 ...
				 * 普通的Controller请求与静态资源请求处理的不同点就再这个地方被分开
				 ...
				 普通请求：RequestMappingHandlerMapping
				 URL请求：SimpleUrlHandlerMapping
